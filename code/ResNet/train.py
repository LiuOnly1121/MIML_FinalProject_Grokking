...
def experiment_on_different_training_data_fraction(p, epochs, training_data_fraction_list, hyper_parameters, batch_size, device):
    ...
    for training_data_fraction in training_data_fraction_list:
        model = MLPResNet(hyper_parameters)
        model.to(device)
        ...
        train_states.extend(train_and_validation(model, epochs, train_dataloader, validation_dataloader, optimizer, criterion, current_path, p, training_data_fraction, device))
